...
function drawBracelet() {
  ...
  for(var i = 0; i < points.length; i++){
    ...
    transform.translate(points[i].x, points[i].y*Math.cos(theta)-points[i].z*Math.sin(theta)+10, points[i].z*Math.cos(theta)+points[i].y*Math.sin(theta));
    cubes.push({
      shape: cube,
      color: new Pre3d.RGBA(HexToR(ColorBeads[i].color)/255,HexToG(ColorBeads[i].color)/255,HexToB(ColorBeads[i].color)/255,1),
      trans: transform});
  }
